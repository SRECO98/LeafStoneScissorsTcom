...
class ListenerToStatus {
        ...
        register2 = firstRoomRefDoc.addSnapshotListener { value, _ ->  //find a way to remove listenr if this down remove desont work!
            ...
            if(statusOfRoom == "close"){
                ...
                if(playerFirstOrSecond == "first"){
                    matchmakeForCompGroup(playerName, firstRoomRefDoc.collection("games"),1)
                    matchmakeTwoPlayers?.updateTextViewConnecting(boolean = true, text = "Connecting ...")
                }else if(playerFirstOrSecond == "second"){
                    matchmakeTwoPlayers?.updateTextViewConnecting(boolean = true, text = "Connecting ...")
                    startDelay(playerName, firstRoomRefDoc.collection("games"),2)
                }
            }
        }
    }

    private fun matchmakeForCompGroup(playerName: String, secondRoomGroupTour: CollectionReference, playerCheck: Int){
        Log.i("TAG", "Function matchmakeForCompGroup started.")
        val query = secondRoomGroupTour.whereEqualTo("status", "open").limit(1)
        query.get()
            .addOnSuccessListener { documents ->
                if (playerCheck == 1){
                    // No open rooms found, create a new room
                    matchmakeTwoPlayers?.callCreateRoom(playerName, secondRoomGroupTour, "group")
    ...
    private fun startDelay(playerName: String, secondRoomGroupTour: CollectionReference, playerCheck: Int) {
        Handler(Looper.getMainLooper()).postDelayed({
            matchmakeTwoPlayers?.updateTextViewConnecting(boolean = false)
            matchmakeForCompGroup(playerName, secondRoomGroupTour, playerCheck)
    ...
    interface MatchmakeTwoPlayers{
        fun callCreateRoom(playerName: String, secondRoomGroupTour: CollectionReference, typeOfGame: String)
